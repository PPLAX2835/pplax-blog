  ...
  <div class="app-container">
    ...
    <div style="margin-top: 5px">
      <el-table border :data="tableData" style="width: 100%" :default-sort="{ prop: 'sort', order: 'descending' }"
        ...
        <el-table-column width="180" align="center" label="留言人">
...
<script>
import { getChatMessageList, deleteChatMessage, deleteChatMessageBatch } from "../../api/chatMessage";
...
export default {
  ...
  computed: {
    ...
    canDeleteBatch: function () {
      return hasAuth(this.menu, 'DELETE:/api/admin/chatMessage')
    },
    /**
     * 检查是否有删除的权限
     * @returns {boolean|*}
     */
    canDelete: function () {
      return hasAuth(this.menu, 'DELETE:/api/admin/chatMessage/{uid}')
    },
  },
  created() {
      ...
      this.$confirm('是否确定删除？', '提示', {
        ...
        if (scope.row === undefined) {
          ...
          if (this.multipleSelection.length) {
            ...
            deleteChatMessageBatch(uids).then(res => {
              this.fetchChatMessageList()
          ...
          // 走单独删除
            deleteChatMessage(scope.row.uid).then(res => {
            this.fetchChatMessageList()
            this.$message.success('删除成功');
            this.loading.close()
          }).catch(() => {
